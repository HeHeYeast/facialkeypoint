...
def draw_landmarks(img,output,draw_circle):
    ...
    for i in range(int(output.shape[0]/2)):
        ...
        if 67>= i >=60:
            ...
            dict_landmarks['in_lip'].append([int(x),int(y),(255,0,0)])
            if draw_circle:
                cv2.circle(img, (int(x),int(y)), 2, (255,0,0),-1)
        '''elif 97>= i >=96:
